...
def sync_activities_from_garmin(
    ...
    try:
        ...
        with session_scope() as session:
            for activity_data in activities:
                try:
                    ...
                    existing_activity = session.query(Activity).filter_by(
                        garmin_activity_id=garmin_id
                    ).first()
                    
                    if existing_activity:
                        # Update existing activity
                        updated = update_activity_from_garmin(
                            session, existing_activity, activity_data, client
                        )
                        if updated:
                            results["activities_updated"] += 1
                    else:
                        # Create new activity
                        new_activity = create_activity_from_garmin(
                            session, activity_data, client
                        )
                        if new_activity:
        ...
        results["success"] = True
        logger.info(f"Sync completed successfully. Processed: {results['activities_processed']}, New: {results['activities_new']}, Updated: {results['activities_updated']}")
    ...
    try:
        ...
        activity = Activity(
            ...
            start_time_utc=datetime.fromisoformat(
                activity_data.get("startTimeGMT", "").replace("Z", "+00:00")
            ) if activity_data.get("startTimeGMT") else None,
...
def get_sync_status() -> Dict:
    ...
    try:
        with session_scope() as session:
            ...
            latest_garmin_activity = session.query(Activity).filter_by(
                source="garmin_connect"
            ).order_by(Activity.start_time_utc.desc()).first()
            ...
            return {
                ...
                "latest_garmin_sync_date": latest_garmin_activity.start_time_utc.isoformat() if latest_garmin_activity else None,
