...
class ActivityParser:
                ...
                for i, point in enumerate(tcx.trackpoints):
                    try:
                        sample = SampleData(
                            ...
                            longitude=float(point.longitude) if hasattr(point, 'longitude') and point.longitude else None,
                            altitude_m=float(point.elevation) if hasattr(point, 'elevation') and point.elevation else None,
    ...
    def _derive_metrics(activity_data: ActivityData):
        ...
        try:
            ...
            if (activity_data.distance_m and activity_data.elapsed_time_s and 
                activity_data.elapsed_time_s > 0 and not activity_data.avg_speed_mps):
