...
# Import will be mocked to avoid garmin_models conflicts
...
def test_data_availability():
    ...
    # Mock the query functions with test data
    mock_activity = {
        "id": 1,
...
def test_chart_metrics_data():
    ...
    with patch("app.data.web_queries.get_activity_samples", return_value=mock_samples):
        ...
        # Basic assertion - we should have both HR and speed data
        assert has_hr, "Heart rate data should be available"
        assert has_speed, "Speed data should be available"
...
def test_activity_page_components():
    ...
    # Assert that the component structure concepts are valid
    assert True  # Basic structural test passes


if __name__ == "__main__":
    # Run with pytest instead of custom test runner
