...
class TestDatabaseModels:
    ...
    def test_activity_model_creation(self, db_session):
        ...
        # Test retrieval
        found = db_session.scalars(select(Activity).where(Activity.external_id == "test_001")).first()
    ...
    def test_sample_model_creation(self, db_session):
        ...
        # Test relationship
        found_activity = db_session.scalars(select(Activity).where(Activity.id == activity.id)).first()
    ...
    def test_route_point_model(self, db_session):
        ...
        # Create route points
        points = [(52.5200, 13.4050, 100.0), (52.5210, 13.4060, 105.0), (52.5220, 13.4070, 110.0)]

        for i, (lat, lon, alt) in enumerate(points):
            route_point = RoutePoint(activity_id=activity.id, sequence=i, latitude=lat, longitude=lon, altitude_m=alt)
    ...
    def test_database_indexes(self, db_session):
        ...
        running_activities = db_session.scalars(select(Activity).where(Activity.sport == "running")).all()

        assert len(running_activities) == 5

        # Test date-based query (uses composite index)
        date_filtered = db_session.scalars(
            select(Activity).where(Activity.sport == "cycling", Activity.start_time_utc >= datetime.now(timezone.utc))
        ).all()
...
class TestDatabaseConfig:
    ...
    def test_session_scope_context_manager(self):
        ...
        with db_config.session_scope() as session:
            found = session.scalars(select(Activity).where(Activity.external_id == "context_test")).first()
            assert found is not None
    ...
    def test_session_scope_rollback_on_exception(self):
        ...
        with db_config.session_scope() as session:
            found = session.scalars(select(Activity).where(Activity.external_id == "rollback_test")).first()
