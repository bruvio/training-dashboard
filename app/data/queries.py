...
class ActivityQueries:
    ...
    def get_sport_distribution(
        ...
        query = select(
            ...
            func.sum(Activity.distance_m).label('total_distance_m'),
            func.sum(Activity.elapsed_time_s).label('total_time_s')
        ).group_by(Activity.sport).order_by(desc(func.count(Activity.id)))
    ...
    def search_activities(
        ...
        ).order_by(desc(Activity.start_time_utc)).limit(limit)
...
class RoutePointQueries:
    ...
    def get_activity_route(
        ...
        """
        query = select(RoutePoint.latitude, RoutePoint.longitude).where(
            RoutePoint.activity_id == activity_id
