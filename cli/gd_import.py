...
def import_activities(
    ...
    force_reimport: bool = typer.Option(
        False, 
        "--force", 
        help="🔄 Reimport all files, ignoring duplicates"
    ),
    ...
    database_url: Optional[str] = typer.Option(
        None,
        "--database-url",
        help="🗄️ Custom database URL (default: sqlite:///garmin_dashboard.db)"
    ),
    ...
    import_results = import_files_with_progress(
        activity_files, 
    ...
    with Progress(
        ...
        for file_path in files:
            try:
                ...
                progress.update(
                    import_task,
                    description=f"Importing {file_path.name}..."
                )
